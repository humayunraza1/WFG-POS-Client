  ...
  generateLogo(isForBrowser = false) {
    if (isForBrowser) {
      ...
      return `<div style="text-align: center; margin-top: 10px; margin-bottom: -30px;">
                ...
                <div style="display: none; font-size: 14px; font-weight: bold; color: #FF1493;">🧇 Fruits Lab 🧇</div>
              </div>`;
    } else {
...
generateRestaurantCopy(order) {
  ...
  lines.push(this.centerText('Fruits Lab'));
  ...
  async printRestaurantCopy(order) {
    ...
    const printContent = `
      ...
      <html>
      <head>
        <title>Restaurant Receipt - Fruits Lab</title>
        <style>
...
async printBothReceipts(order) {
  ...
  const printContent = `
    ...
    <html>
    <head>
      <title>Receipt - Fruits Lab</title>
      <style>
